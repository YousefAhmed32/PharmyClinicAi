...
function ArticleModal({ editing, showModal, onClose, onSave, isSaving }) {
  ...
  const validate = () => {
    ...
    if (!(form.summary || "").trim() || form.summary.length < 10) e.summary = "Min 10 characters";
    if (!(form.content || "").trim() || form.content.length < 50) e.content = "Min 50 characters";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-50 bg-white rounded-2xl shadow-lifted w-full max-w-2xl max-h-[92vh] flex flex-col animate-scale-in">
        <div className="flex items-center justify-between p-5 border-b border-neutral-100 shrink-0">
          <h2 className="font-display font-semibold text-lg">{editing ? "Edit Article" : "New Article"}</h2>
        ...
        <div className="flex border-b border-neutral-100 px-5 shrink-0">
          {[["basic","📝 Basic Info"],["content","📄 Content"]].map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)}
              className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors -mb-px ${tab===id?"border-primary-600 text-primary-700":"border-transparent text-neutral-500 hover:text-neutral-700"}`}>
              {label}
            </button>
          ))}
        </div>
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-5 space-y-4">
            {tab === "basic" && (
              <>
                ...
                <div>
                  <label className="label">Summary * <span className="text-neutral-400 font-normal text-xs">(shown on listing page)</span></label>
                  <textarea className={`input ${errors.summary?"input-error":""}`} rows={2} value={form.summary} onChange={f("summary")} placeholder="Brief description (max 500 chars)…"/>
                ...
                <div>
                  <label className="label">Tags <span className="text-neutral-400 font-normal text-xs">(comma separated)</span></label>
                  <input className="input" value={form.tags} onChange={f("tags")} placeholder="vitamins, health, immunity…"/>
                </div>
              </>
            )}
            {tab === "content" && (
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs text-neutral-400">{wordCount} words · ~{readTime} min read</span>
                </div>
                <textarea className={`input font-mono text-sm leading-relaxed ${errors.content?"input-error":""}`} rows={18}
                  value={form.content} onChange={f("content")} placeholder="Write your article here… (min 50 characters)"/>
                {errors.content && <p className="error-text">{errors.content}</p>}
                <p className="text-xs text-neutral-400 mt-1.5">💡 Tip: Use blank lines to separate paragraphs for better readability.</p>
              </div>
            )}
          </div>
          <div className="flex justify-between items-center px-5 py-4 border-t border-neutral-100 bg-neutral-50 rounded-b-2xl shrink-0">
            <div className="flex gap-2">
              {tab==="content" && <button type="button" onClick={() => setTab("basic")} className="btn-ghost btn-sm">← Basic Info</button>}
              {tab==="basic"   && <button type="button" onClick={() => setTab("content")} className="btn-secondary btn-sm">Content →</button>}
            </div>
            <div className="flex gap-2">
              ...
              <button type="submit" disabled={isSaving} className="btn-primary min-w-[110px]">
                {isSaving ? "Saving…" : form.status==="published" ? "🌐 Publish" : "💾 Save Draft"}
              </button>
            </div>
          </div>
        </form>
...
export default function AdminBlogPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success(editing?"Article updated ✓":"Article created ✓"); qc.invalidateQueries(["admin-blog"]); qc.invalidateQueries(["admin-blog-stats"]); closeModal(); },
    onError: (err) => toast.error(err.response?.data?.message || "Failed to save"),
  });

  const remove = useMutation({
    mutationFn: (id) => blogAPI.delete(id),
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries(["admin-blog"]); },
    onError: (err) => toast.error(err.response?.data?.message || "Delete failed"),
  });

  const togglePublish = useMutation({
    mutationFn: (a) => a.status==="published" ? blogAPI.unpublish(a._id) : blogAPI.publish(a._id),
    onSuccess: (_,a) => { toast.success(a.status==="published"?"Moved to draft":"Published ✓"); qc.invalidateQueries(["admin-blog"]); },
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-0.5">{meta.total||0} total articles</p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          {[{label:"Total",val:stats.total,icon:"📝",color:"bg-neutral-50"},{label:"Published",val:stats.byStatus?.published||0,icon:"🌐",color:"bg-primary-50"},{label:"Drafts",val:stats.byStatus?.draft||0,icon:"📋",color:"bg-yellow-50"},{label:"Top Views",val:stats.topViewed?.[0]?.views||0,icon:"👁️",color:"bg-blue-50"}].map(({label,val,icon,color})=>(
      ...
      <div className="card p-4 mb-5 space-y-3">
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input className="input pl-9 text-sm" placeholder="Search title or summary..." value={searchVal} onChange={e=>{setSearchVal(e.target.value);debouncedSearch(e.target.value);}}/>
          </div>
        ...
        {hasFilters && (
          <div className="flex flex-wrap gap-2">
            ...
            {filters.category && <span className="badge-gray text-xs capitalize">{filters.category.replace("-"," ")}</span>}
            {filters.status   && <span className={filters.status==="published"?"badge-green text-xs":"badge-yellow text-xs"}>{filters.status}</span>}
          </div>
        )}
      </div>
      ...
      <div className={`card transition-opacity ${isFetching?"opacity-70":""}`}>
        {isLoading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="font-semibold text-neutral-700">No articles found</p>
            {hasFilters ? <button onClick={resetFilters} className="btn-secondary mt-4">Clear Filters</button> : <button onClick={openCreate} className="btn-primary mt-4">Write First Article</button>}
          </div>
        ) : (
          <div className="table-wrapper rounded-none border-0">
            <table className="table">
              <thead><tr><th>Article</th><th>Category</th><th>Author</th><th>Views</th><th>Status</th><th>Published</th><th>Actions</th></tr></thead>
              <tbody>
                {articles.map(a => (
                  <tr key={a._id}>
                    <td>
                    ...
                    <td><span className="badge-gray text-xs capitalize">{a.category?.replace("-"," ")}</span></td>
                    ...
                    <td>
                      <div className="flex gap-1">
                        <button onClick={() => openEdit(a)} className="btn-ghost btn-sm text-blue-600">Edit</button>
                        <button onClick={() => { if(window.confirm(`Delete "${a.title}"?`)) remove.mutate(a._id); }} className="btn-ghost btn-sm text-red-500 hover:bg-red-50">Del</button>
                      </div>
                    </td>
                  </tr>
                ))}
