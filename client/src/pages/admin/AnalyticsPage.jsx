  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-neutral-900">{t('analytics.title')}</h1>
          <p className="text-neutral-500 text-sm mt-1">{t('analytics.subtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: t('analytics.totalRevenue'),   val: `${totalRevenue.toLocaleString()} ${t('common.currency')}`, icon:'💰', color:'bg-primary-50 text-primary-700' },
          { label: t('analytics.totalOrders'),    val: totalOrders,                                                  icon:'📦', color:'bg-blue-50 text-blue-700' },
          { label: t('analytics.avgOrderValue'),  val: `${avgOrderValue.toFixed(0)} ${t('common.currency')}`,        icon:'📊', color:'bg-purple-50 text-purple-700' },
          { label: t('analytics.totalPatients'),  val: overview?.users.total || '—',                                 icon:'👥', color:'bg-orange-50 text-orange-700' },
          ...
          <div key={label} className="card p-5">
            ...
            <p className="text-xs text-neutral-500 mt-1">{label} — {t('analytics.lastDays', { days })}</p>
          </div>
        ))}
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-neutral-800">{t('analytics.revenueTrend', { days })}</h2>
          <p className="text-sm text-neutral-500">{totalRevenue.toLocaleString()} {t('common.currency')} {t('common.total')}</p>
        </div>
        {revenue
          ? <BarChart data={revenue} valueKey="revenue" labelKey="date" color="bg-primary-500" height={140} formatVal={v=>`${v.toLocaleString()} ${t('common.currency')}`}/>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-neutral-800">{t('analytics.ordersPerDay', { days })}</h2>
          <p className="text-sm text-neutral-500">{totalOrders} {t('analytics.totalOrders')}</p>
        </div>
        {ordersTrend?.byDay
          ? <BarChart data={ordersTrend.byDay} valueKey="count" labelKey="_id" color="bg-blue-400" height={100} formatVal={v=>`${v} ${t('analytics.totalOrders')}`}/>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <div className="card p-6">
          <h2 className="font-semibold text-neutral-800 mb-5">{t('analytics.ordersByStatus')}</h2>
        ...
        <div className="card p-6">
          <h2 className="font-semibold text-neutral-800 mb-5">{t('analytics.revenueByCategory')}</h2>
          {catDonut.length > 0
      ...
      <div className="card">
        <div className="px-6 py-4 border-b border-neutral-100">
          <h2 className="font-semibold text-neutral-800">{t('analytics.top10Products')}</h2>
        </div>
            ...
            <tbody>
              {(topProducts || []).map((p, i) => (
                <tr key={p._id}>
                  ...
                  <td><span className="badge-gray capitalize text-xs">{t(`store.categories.${p.category}`) || p.category?.replace('-',' ')}</span></td>
                  <td><span className="font-semibold text-neutral-800">{p.sold}</span></td>
                  <td><span className="font-semibold text-primary-700">{p.revenue?.toLocaleString()} {t('common.currency')}</span></td>
                  <td>
      ...
      <div className="card">
        <div className="px-6 py-4 border-b border-neutral-100">
          <h2 className="font-semibold text-neutral-800">{t('analytics.categoryPerformance')}</h2>
        </div>
        <div className="table-wrapper rounded-none border-0">
          <table className="table">
            <thead>
              <tr>
                <th>{t('admin.categoryLabel')}</th>
                <th>{t('analytics.products')}</th>
                <th>{t('analytics.totalStock')}</th>
            ...
            <tbody>
              {(categories || []).map(cat => (
                <tr key={cat.category}>
                  <td><span className="badge-gray capitalize">{t(`store.categories.${cat.category}`) || cat.category?.replace('-',' ')}</span></td>
                  <td>{cat.products}</td>
                  <td>{cat.totalStock}</td>
                  <td>{cat.avgPrice} {t('common.currency')}</td>
                  <td>{cat.sold}</td>
                  <td className="font-semibold text-primary-700">{cat.revenue?.toLocaleString()} {t('common.currency')}</td>
                </tr>
              ))}
            </tbody>
          </table>
